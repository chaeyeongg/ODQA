...
class SparseRetrieval:
    ...
    def get_sparse_embedding(self) -> None:
        ...
        # 파일명: tfidf_embedding_문서개수.pkl
        cache_file = os.path.join(cache_dir, f"tfidf_embedding_{len(self.contexts)}.pkl")

        # 2. 캐시 확인 및 로드
        if os.path.isfile(cache_file):
            print(f"✅ Loading TF-IDF embedding from cache: {cache_file}")
            with open(cache_file, "rb") as f:
                self.p_embedding = pickle.load(f)
        else:
            # 3. 없으면 생성 (TF-IDF 벡터화 수행)
            print("🚀 Building TF-IDF embedding (This may take a while)...")
            self.p_embedding = self.tfidfv.fit_transform(self.contexts)

            # 4. 저장
            print(f"💾 Saving TF-IDF embedding to cache: {cache_file}")
            with open(cache_file, "wb") as f:
                pickle.dump(self.p_embedding, f)

        print("TF-IDF embedding build complete.")
...
class BM25Retrieval:
    def __init__(
        ...
        self.bm25 = None  # get_sparse_embedding() 호출 시 생성
...
class HybridRetrieval:
    def __init__(self, sparse_retriever, dense_retriever):
        ...
        # 두 Retrieval의 문서 개수가 서로 동일한지 확인
    ...
    def retrieve(
        ...
        alpha=1.0: Only Dense / alpha=0.0: Only BM25
        """
        # Case A: 단일 질문
        if isinstance(query_or_dataset, str):
    ...
    def get_relevant_doc(self, query: str, k: int = 10, alpha: float = 0.5) -> Tuple[List, List]:
        ...
        dense_scores = torch.matmul(dense_q_emb, self.dense.p_embedding.T).squeeze(0).cpu().numpy() # [1, num_docs] -> [num_docs]
...
class RerankRetrieval:
    def __init__(self, base_retriever, model_name_or_path: str = "BAAI/bge-reranker-v2-m3", device: str = "cuda"):
        ...
        Reranker 초기화
        """
        self.base_retriever = base_retriever # 1차 검색 Retrieval
        self.device = device
    ...
    def retrieve(self, query_or_dataset, topk=10, **kwargs):            
            ...
            candidate_k = 50 # Recall 실험 후 최적의 값으로 fix
